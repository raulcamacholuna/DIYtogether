...
# Último cambio: Implementación del módulo para el botón 'Evo Viento' (EVO_4).
# Descripción: Encapsula la lógica del botón 'Evo Viento', incluyendo creación, posicionamiento en el panel lateral, icono (ASSET_ICON_EVO_WIND) y conexión de evento a la acción ACTION_ID_EVO_WIND.
*/
#include "evo_4.h"
...
void evo_4_create(lv_obj_t *parent) {
    ...
    lv_img_set_src(img, ui_assets_get_icon(ASSET_ICON_EVO_WIND));
    lv_obj_center(img);

    // --- Posición inicial (para animación de entrada) ---
    // El índice de este botón es 3 en el panel lateral.
    lv_obj_set_pos(s_evo_4_handle, -BUTTON_SIZE, BUTTON_PADDING + (BUTTON_SIZE + BUTTON_PADDING) * 3);
    lv_obj_add_flag(s_evo_4_handle, LV_OBJ_FLAG_HIDDEN);
