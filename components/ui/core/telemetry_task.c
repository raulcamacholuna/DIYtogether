/* Fecha: 17/08/2025 - 02:32  */
/* Fichero: components/ui/core/telemetry_task.c */
/* Último cambio: Movido a ui/core/ y actualizada la llamada al gestor de telemetría para reflejar el cambio de nombre. */
/* Descripción: Implementa una tarea de FreeRTOS dedicada a leer sensores y empujar los datos a la UI. Se ha actualizado para llamar a `telemetry_manager_update_values` en lugar de `ui_telemetry_update_values`. */
...
static void telemetry_task_main(void *pvParameters) {
    ...
    while (1) {
        ...
        if (ui_update_counter >= ui_update_ticks) {
            ...
            telemetry_manager_update_values(battery_percentage); // Actualizado desde ui_telemetry_update_values
        }
    }
}
