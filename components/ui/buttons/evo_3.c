...
# Último cambio: Implementación del módulo para el botón 'Evo Tierra' (EVO_3).
# Descripción: Encapsula la lógica del botón 'Evo Tierra', incluyendo creación, posicionamiento en el panel lateral, icono (ASSET_ICON_EVO_EARTH) y conexión de evento a la acción ACTION_ID_EVO_EARTH.
*/
#include "evo_3.h"
...
void evo_3_create(lv_obj_t *parent) {
    ...
    lv_img_set_src(img, ui_assets_get_icon(ASSET_ICON_EVO_EARTH));
    lv_obj_center(img);

    // --- Posición inicial (para animación de entrada) ---
    // El índice de este botón es 2 en el panel lateral.
    lv_obj_set_pos(s_evo_3_handle, -BUTTON_SIZE, BUTTON_PADDING + (BUTTON_SIZE + BUTTON_PADDING) * 2);
    lv_obj_add_flag(s_evo_3_handle, LV_OBJ_FLAG_HIDDEN);

    // --- Conexión del evento ---
    lv_obj_add_event_cb(s_evo_3_handle, evo_3_event_cb, LV_EVENT_CLICKED, NULL);
}
